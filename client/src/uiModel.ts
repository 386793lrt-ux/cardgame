...
export type PendingAction = "PLAY_CARD" | "ATTACK" | "END_TURN" | "SURRENDER" | null;
...
export function isCardPlayable(game: PlayerViewState, card: CardView, pending: PendingAction): boolean {
  return pending === null && isYourTurn(game) && card.cost <= game.you.mana;
}
...
export function presentationFromEvents(
  ...
  const played = events.find((event): event is Extract<GameEvent, { type: "CARD_PLAYED" }> => event.type === "CARD_PLAYED" && event.playerId === viewerId);
  ...
  return {
    ...
    playedCard: played ? oldHand.find((card) => card.instanceId === played.cardInstanceId) : undefined,
