...
export function Hand({ game, selectedId, inspectedCard, drawnCardId, pending, onInspect, onPlay }: {
    ...
    <section className="hand-dock" aria-label="你的手牌">
      ...
      <div className="hand-zone">
        {game.you.hand.map((card, index) => {
          ...
          return (
            <div key={card.instanceId} className="hand-card-slot" onMouseEnter={() => onInspect(card)} onMouseLeave={() => onInspect(selectedId === card.instanceId ? card : null)}>
              <Card card={card} index={index} selected={selectedId === card.instanceId} playable={isCardPlayable(game, card, pending)} drawn={drawnCardId === card.instanceId} fanAngle={centered * spread} fanOffset={centered * Math.min(22, 150 / Math.max(1, count))} onClick={() => onPlay(card)} />
            </div>
          );
        })}
      </div>
