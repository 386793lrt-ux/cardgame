...
import { createGame, createPlayerView, executeAction, type GameState } from "@riftbound/game-core";
import type { PlayerAction, PlayerViewState } from "@riftbound/shared";
import { RuleBasedGameAI } from "./GameAI.js";

const ai = new RuleBasedGameAI();

function initial(seed = 7): GameState {
  return createGame(`GAME_${seed}`, `ROOM_${seed}`, [{ playerId: "A", name: "甲" }, { playerId: "B", name: "乙" }], seed).state;
}
...
test("AI 有斩杀法术时优先攻击英雄", () => {
  ...
  me.hand = [{ instanceId: "LETHAL_SPELL", definitionId: "CARD_000015" }];
  me.mana = 2;
  enemy.health = 3;
  const action = choose(state, 1);
